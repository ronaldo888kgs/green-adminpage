...
export function CollectionTemplate({
  ...
  const metadataBanner = banner
  const [selectedBanner, setSelectedBanner] = useState(metadataBanner || notFound)
  const onBannerError = () => {
    setSelectedBanner(notFound)
  }

  const detailView = () =>{
    history.push({ pathname: '/collection/detail', state: {image: image, name: name, description: description, status: status, create_date: create_date}});
  }
  return (
    <S.Row>
      <S.Content>
        ...
        <div style={{height: '50px', width: '50%', borderLeft: '1px solid #c8ced3', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
          <textarea style={{width: '100%', border: '0', textAlign: 'center', alignContent: 'middle'}} readOnly>{description}</textarea>          
        </div>
        <div style={{height: '50px', width: '70px', borderLeft: '1px solid #c8ced3'}}>
          <S.Img
            src={imgLH3(selectedBanner, 400)}
            className={selectedBanner === notFound ? 'img-fail' : ''}
