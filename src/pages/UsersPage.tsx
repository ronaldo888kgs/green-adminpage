...
import { UserTemplate } from '../components/shared/template/rows/UserTemplate'
...
export default function UsersPage() {
  ...
  const { loading, hasMore, loadMore, users } = useUsers(searchName, 20)
  const loader = (
    <S.CardsContainer>
  ...
  return (
    <TablePageTemplate>
      ...
      <S.TableDiv>
        <InfiniteScroll next={loadMore} hasMore={hasMore} loader={loader} dataLength={users.length} style={{border: '1px solid #c8ced3'}}>
          <S.CardsContainer>
            <S.Header>
              <div>
                Image
            ...
            {users.map(user => {
              const image = API.server_url + API.user_profile_image + String(user?.image)
              return (
                <UserTemplate
                  key={`${user.id}`}
                  id={user.id}
